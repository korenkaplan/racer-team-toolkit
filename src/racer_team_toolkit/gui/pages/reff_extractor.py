...
class DeviceTimeWorker(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.status.emit(f"PC Time: {pc_datetime:%d-%m-%Y %H:%M:%S}")
    ...
    def run(self) -> None:
        ...
        try:
            for device_info in self.incorrect_devices:
                ...
                self.status.emit(f"REFF files found: {reff_count}")
                self.status.emit(f"Screen videos found: {video_count}")

                if not corrections:
                    self.status.emit("No affected files found for this device.")
                    corrected_devices.append(device)
                    continue

                for correction in corrections:
                    current = datetime.fromtimestamp(correction.current_timestamp)
                    corrected = datetime.fromtimestamp(correction.corrected_timestamp)
                    self.status.emit(
                ...
                self.status.emit(f"Applying {len(corrections)} correction(s)...")
                ...
                if corrected_count == len(corrections):
                    self.status.emit(f"✓ {device.name}: {corrected_count} file(s) corrected.")
                    corrected_devices.append(device)
                else:
                    self.status.emit(
                        f"✗ {device.name}: "
                    ...
                    self.status.emit("Device will be skipped during extraction.")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if self.include_videos:
                self.status.emit("Grouping screen videos into flights...")
            ...
            self.finished.emit(
                {
                    ...
                    "include_videos": self.include_videos,
                    "device_types": tuple(device.file_prefix for device in self.devices),
...
class ReffExtractorPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.reff_only_button.clicked.connect(lambda: self._select_mode(False))

        self.reff_video_button = QPushButton("REFF & Screen Videos")
        self.reff_video_button.setObjectName("modeButton")
        self.reff_video_button.setCheckable(True)
        self.reff_video_button.clicked.connect(lambda: self._select_mode(True))
        ...
        self.time_table.setSelectionMode(QTableWidget.SelectionMode.NoSelection)
        ...
        self.skip_bad_time_button = QPushButton("Skip Incorrect Devices")
        self.skip_bad_time_button.setObjectName("secondaryButton")
        self.skip_bad_time_button.hide()
        self.skip_bad_time_button.clicked.connect(self._skip_incorrect_devices)

        self.correct_time_button = QPushButton("Correct Affected Files")
        self.correct_time_button.setObjectName("primaryButton")
        self.correct_time_button.hide()
        self.correct_time_button.clicked.connect(self._correct_times)
        ...
        self.log.setPlaceholderText("Extraction and transfer activity will appear here...")
        ...
        self.results_summary = QLabel("Run an extraction to see grouped flights.")
        ...
        self.flight_table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
        self.flight_table.setSelectionMode(QTableWidget.SelectionMode.NoSelection)
    ...
    def _select_mode(self, include_videos: bool) -> None:
        ...
        self.include_videos = include_videos
        self.reff_only_button.setChecked(not include_videos)
        self.reff_video_button.setChecked(include_videos)
    ...
    def refresh_devices(self) -> None:
        ...
        self.time_worker.status.connect(self._append_log)
        self.time_worker.finished.connect(self._show_time_status)
        self.time_worker.failed.connect(self._time_failed)
        self.time_worker.finished.connect(self.time_thread.quit)
        self.time_worker.failed.connect(self.time_thread.quit)
        self.time_thread.finished.connect(self._cleanup_time_thread)
    ...
    def _show_time_status(
        ...
        info_by_serial = {item.device.serial: item for item in time_info}

        self.time_table.setRowCount(len(devices))

        for row, device in enumerate(devices):
            info = info_by_serial.get(device.serial)
        ...
        if incorrect:
            self.time_status.setText(f"{len(incorrect)} need fix")
            ...
            self._append_log("")
            self._append_log(f"⚠ {len(incorrect)} device(s) have an incorrect clock.")
        else:
    ...
    def _skip_incorrect_devices(self) -> None:
        ...
        self._append_log("Continuing without devices that have incorrect clocks.")

        self.extract_button.setEnabled(bool(self.devices_to_process))

    def _correct_times(self) -> None:
        """Correct affected files before extraction."""

        if self.correction_thread is not None and self.correction_thread.isRunning():
            return
        ...
        self.correction_worker = TimeCorrectionWorker(self.incorrect_devices)
        ...
        self.correction_worker.status.connect(self._append_log)
        self.correction_worker.finished.connect(self._corrections_finished)
        self.correction_worker.failed.connect(self._correction_failed)
        self.correction_worker.finished.connect(self.correction_thread.quit)
        self.correction_worker.failed.connect(self.correction_thread.quit)
        self.correction_thread.finished.connect(self._cleanup_correction_thread)
    ...
    def _corrections_finished(
        ...
        self.devices_to_process = correct_devices + corrected_devices
    ...
    def _correction_failed(self, message: str) -> None:
        ...
        self._append_log(f"✗ Time correction failed: {message}")
    ...
    def start_extraction(self) -> None:
        ...
        if self.extraction_thread is not None and self.extraction_thread.isRunning():
            return
        ...
        self.extraction_worker.status.connect(self._append_log)
        self.extraction_worker.progress.connect(self._update_progress)
        self.extraction_worker.finished.connect(self._show_results)
        self.extraction_worker.failed.connect(self._extraction_failed)
        self.extraction_worker.finished.connect(self.extraction_thread.quit)
        self.extraction_worker.failed.connect(self.extraction_thread.quit)
        self.extraction_thread.finished.connect(self._cleanup_extraction_thread)
    ...
    def _show_results(self, result: dict) -> None:
        ...
        self.flight_table.setColumnCount(len(headers))
        self.flight_table.setHorizontalHeaderLabels(headers)
        ...
        if include_videos:
            summary_parts.append(f"Videos {transfer_verb}: {result['copied_videos']}")
        ...
        if include_videos:
            self._append_log(f"Screen videos {transfer_verb}: {result['copied_videos']}")

        if warnings:
            self._append_log(f"Recording warnings: {len(warnings)}")
            self._append_log("")
        ...
        self._append_log("")
        self._append_log(f"All files are located at: {result['destination']}")
    ...
    def _extraction_failed(self, message: str) -> None:
        ...
        self._append_log(f"✗ Extraction failed: {message}")
        self.activity_status.setText("Failed")
    ...
    def _time_failed(self, message: str) -> None:
        ...
        self._append_log(f"✗ Device check failed: {message}")
